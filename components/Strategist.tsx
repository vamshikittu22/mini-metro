...
export const Strategist: React.FC<StrategistProps> = ({ gameState, onClose }) => {
  ...
  const analyzeGame = async () => {
    ...
    try {
      ...
      const prompt = `Analyze this Mini Metro game state and suggest 3 strategic improvements. Be concise, expert, and encouraging. Use bullet points.
  ...
  return (
    <div className="fixed right-8 top-32 z-[100] w-80 bg-white border-2 border-black p-6 shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] rounded-sm flex flex-col gap-4 animate-in fade-in slide-in-from-right-4">
      <div className="flex justify-between items-center border-b-2 border-black pb-2">
        <h3 className="text-xs font-black uppercase tracking-widest text-black">Strategic Advisor</h3>
      ...
      {loading && (
        <div className="py-8 flex flex-col items-center gap-4">
          ...
          <span className="text-[10px] font-black uppercase text-black">Analyzing Network...</span>
        </div>
      )}

      {error && (
        <div className="text-red-600 text-[10px] font-bold uppercase leading-relaxed">
          {error}
        </div>
      )}

      {advice && !loading && (
        <div className="text-[11px] font-bold leading-relaxed max-h-[400px] overflow-y-auto no-scrollbar text-black">
          {advice.split('\n').map((line, i) => (
